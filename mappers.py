...
class MetafileMapper(IdMapper):
    def __init__(self, naan: str, shoulders: List[str], db_path: Path) -> None:
        self._naan = naan
    ...
    def get_url(self, ark: ArkIdentifier) -> str:
        if (not self._db_path.exists()
                ...
                or not os.access(self._db_path, os.R_OK)):
            raise MapperWarning(f"Non accessible database {self._db_path.absolute()}")
        conn = sqlite3.connect(self._db_path)
        cur = conn.cursor()
        cur.execute(
            "SELECT url FROM file_cache WHERE ark_id = ?",
            (str(ark),)
        )
        row = cur.fetchone()
        if row is None:
            raise ResolveError(f"Unregistered ARK identifier `{str(ark)}`")
        return row[0]

    def get_metadata(self, ark: ArkIdentifier, fmt: MetadataFormat) -> str:
        if (not self._db_path.exists()
                or not self._db_path.is_file()
                or not os.access(self._db_path, os.R_OK)):
            raise MapperWarning(f"Non accessible database {self._db_path.absolute()}")
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT metadata_rdf FROM file_cache WHERE ark_id = ?",
            (str(ark),)
        )
        row = cur.fetchone()
        if row is None:
    ...
    def is_responsible(self, ark: ArkIdentifier):
        return ark.naan == self._naan and ark.shoulder in self._shoulders
